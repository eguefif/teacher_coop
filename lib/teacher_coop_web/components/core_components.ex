defmodule TeacherCoopWeb.CoreComponents do
  @moduledoc """
  ...
  def input(%{type: "search"} = assigns) do
    ~H"""
    <div class="fieldset">
      <label for={@id} class="input input-lg w-100 rounded-xl">
        <span :if={@label} class="label mb-1">{@label}</span>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(TeacherCoopWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(TeacherCoopWeb.Gettext, "errors", msg, opts)
    end
  end
